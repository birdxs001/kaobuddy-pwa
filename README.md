...
- AI 从资料里抽"进程""线程""死锁""银行家算法"这种**知识点**，不给你排日历。
- 按知识点模块推进，学完勾掉，不搞虚的进度条。
- 针对单个知识点出题、批改、讲解——不是泛泛的"本章复习"。
- 考前生成模拟考，可自定义题型和时长，导出 PDF 打印。
- 最后几天切到临考速背模式，卡片正面问题背面答案，左滑不会右滑会。
...
- **邀请码（beta）**：如果你分享了邀请码给朋友，他们可以先不填 Key，用服务端配置的 API 额度体验。用量和预算实时显示。
...
如果资料有多个"P,V 操作"的具体题目（独木桥问题、水果盘问题），AI 会把它们合并成"信号量 P、V 操作"一个模块，具体题目放进考察内容当例题。
...
模拟考批改的错题自动入库，手动也能添加。薄弱点独立管理，带严重程度标记，可定期复习。
